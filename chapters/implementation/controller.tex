...
export class ValidationEngine<T> {
  ...
  setRuleset(ruleset: ValidationRule[]) {
    ...
    \subsubsection{Accessing Data by Evaluating JSONPath Expressions}
      ...
      The ability to access certain information from the runtime scope is needed before making an HTTP request and when evaluation a condition listed . To evaluate a JSONPath expression, the \verb;jsonpath; library is used. 
